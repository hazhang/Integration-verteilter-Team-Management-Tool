...
function startup($param){
	...
	$i = 121;
	while($i < 136){
		$hosts[$i] = "141.19.143.".$i;
		$i++;
	}
/* 	foreach($hosts as $host){
		$data = request($host."/startup.php", $request);
		if(!$data){
			$startedHosts[$host] = $data;
		}
	}
 */	 
...
function request($url, $vars){
	$options = array(
        ...
        CURLOPT_USERAGENT      => "IGT_G16",    // who am i 
...
/**
 *	response the remote client request, return the xml formated 
 *	string with the agentid.
 *
 */
function response($data){
	$data_obj = simplexml_load_string($data);
	$agentid = $data_obj->agent->id;
	$xml = new SimpleXMLElement('<response></response>');
	$agent = $xml->addChild('agent');
	$agent->addChild('id', htmlspecialchars(utf8_encode($agentid)));
	return $xml->asXML();
}
